...
安装启动 [docker](https://www.docker.com/) 之后，在终端运行以下命令即可体验。<br>
  ```shell
   docker run -d --name likeshop --restart=always -p 20208:80 -e MYSQL_ROOT_PASSWORD=root ghcr.io/op4219sr-bot/likeshop:aio-latest
  ```

> `--restart=always` 让容器在**服务器重启后自动启动**，也会在意外退出时自动拉起，强烈建议加上。

如果需要自定义参数永久挂载数据，在终端运行以下命令，其中"【】"改成自定义参数。
```shell
docker run -d --name likeshop --restart=always \
...
5.**`--restart=always` 务必加上**，否则服务器重启后容器不会自动启动，网站会打不开（详见下面「🔄服务器重启后容器自动启动」一节）。<br>
6.不熟悉 docker 请勿用于生产环境，可能造成数据丢失等问题。
...
服务器重启后,浏览器打不开商城,`docker ps` 看不到 likeshop 容器(`docker ps -a` 能看到但状态是 `Exited`)。

### 原因

`docker run` 启动容器时**没有加 `--restart` 参数**,默认策略是 `no`,所以服务器重启后 Docker daemon 起来了,但容器不会跟着起来。
...
```bash
docker update --restart=always likeshop
```

> 这条命令只需要跑**一次**,以后:
> - 服务器重启 → 容器自动起来 ✅
> - 容器意外崩溃 → 自动重启 ✅
> - docker daemon 重启 → 容器自动起来 ✅

验证设置生效:

```bash
docker inspect -f '{{.HostConfig.RestartPolicy.Name}}' likeshop
```

输出 `always` 就 OK 了。

### 服务器现在已经重启过、容器是 Exited 状态怎么办?

```bash
# 1. 手动把容器拉起来
docker start likeshop

# 2. 加上自启策略,避免下次再踩坑
docker update --restart=always likeshop

# 3. 等 30 秒,确认 nginx 起来了
docker logs --tail 20 likeshop
```
...
| 策略 | 服务器重启 | 容器崩溃 | 适用场景 |
|---|---|---|---|
| `no`(默认) | ❌ 不重启 | ❌ 不重启 | 一次性任务、临时测试 |
| `on-failure` | ❌ 不重启 | ✅ 仅非 0 退出码时重启 | 批处理任务 |
| `unless-stopped` | ✅ 自动起 | ✅ 重启 | 推荐:服务一直跑、但允许你手动 `docker stop` 后不重启 |
| `always` | ✅ 自动起 | ✅ 重启 | 推荐:始终保持运行,**生产部署用这个** |

`always` 和 `unless-stopped` 的区别:用 `docker stop likeshop` 手动停止后,`always` 在 docker daemon 重启后仍会拉起来,`unless-stopped` 会保持停止状态。一般部署用 `always` 就行。
